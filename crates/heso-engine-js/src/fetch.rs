...
fn perform_request(
    ...
    if let Some(payload) = heso_engine_fetch::parse_data_url(&req.url) {
...
#[cfg(test)]
mod tests {
    use heso_engine_fetch::parse_data_url;
    ...
    #[test]
    fn javascript_data_url_decodes_to_script_body() {
        let p = parse_data_url("data:text/javascript;base64,aGVsbG8=").expect("parse");
        assert_eq!(p.body, b"hello");
    }
}
